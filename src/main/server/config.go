...
const backendtype kvraft.BackendType = kvraft.Bolt
...
func (cfg *config) StartServer(i int) {
	...
	cfg.kvservers[i] = kvraft.StartKVServer(ends, i, cfg.saved[i], cfg.maxraftstate, backendtype)

	kvsvc := labrpc.MakeService(cfg.kvservers[i])
	rfsvc := labrpc.MakeService(cfg.kvservers[i].GetRaft())
...
func (cfg *config) Leader() (bool, int) {
	...
	for i := 0; i < cfg.n; i++ {
		_, is_leader := cfg.kvservers[i].GetRaft().GetState()
		if is_leader {
			return true, i
		}
	}
